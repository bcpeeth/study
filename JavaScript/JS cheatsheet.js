...
REQUESTS II {
    ...
    1.  //First, call the fetch() function and pass it this URL as a string:
    https://api-to-call.com/endpoint
    // This first argument determines the endpoint of the request.

    fetch('https://api-to-call.com/endpoint')

    2.  //Chain a .then() method to the end of the fetch() function and pass it the success callback arrow function as its first argument. The success callback function takes one parameter, response.
    // .then() will fire only after the promise status of fetch() has been resolved.

    fetch('https://api-to-call.com/endpoint').then(response => {
    }

    3.  //Inside of the response callback function, check the ok property of response inside of a conditional statement. In the code block of the conditional statement, return response.json().
    ...
    4.  //Below the curly braces of the conditional statement, create a new error with this code:
    throw new Error('Request failed!');
    ...
    5.  //Add a second argument to .then(), it will be an arrow function that will handle our failures. Separate the first callback function from the second with a comma. The second callback function takes a single parameter, networkError.
